# Launch Desk — next ideas from Omarchy Quattro plugins

**Purpose:** Prioritized “steal / skip” list for `harris.launch-desk` before marketplace submit.  
...
**Baseline research:** `docs/research/omarchy-plugins.md` (22 Aug) — this doc refreshes official + marketplace standouts and maps gaps onto **current** Launch Desk (v1.2.0).
...
| Plugin | id | kinds | Pattern to steal for Launch Desk |
|--------|-----|-------|----------------------------------|
| **Clock** | `omarchy.clock` | `bar-widget` | Tiny bar text → nested calendar panel; format via bar settings; **archetype Launch Desk already follows**. |
...
| **Emojis / Clipboard / Image picker** | overlays | `overlay` + often `keepLoaded` | Fullscreen on demand; **irrelevant** unless Launch Desk ever grows a patch/image browser. |
...
## 2. Marketplace standouts — features Launch Desk lacks

| Plugin | What they do well | Gap vs Launch Desk today |
|--------|-------------------|---------------------------|
...
| **Normarchy** | Sticky panel while video plays; **Esc hides, audio continues**; Space/M/O/S shortcuts; middle-click stop; tokenized 127.0.0.1 proxy; optional Hypr bind; strong disclaimer | Launch Desk **pauses + stops proxy on panel hide** — opposite sticky model. Buttons exist; **no key bindings** for Space/Mute/Open. |
| **omaStream** | Overlay + service + bar; quality menu; audio-only; Esc leaves fullscreen then closes; **closing overlay keeps playback for bar control**; middle/right/wheel on bar | Overbuilt for a launch desk. Steal: quality enum, sticky playback option, bar transport clicks — not Discover/download. |
...
| **Games generally** | Delight + theme | Skip game loops; Launch Desk’s “game” is countdown + Watch. |
...
| 4 | **T−10 / T−0 toast (opt-in)** | When `notifyMilestones` and countdown crosses 600s / 0: `notify-send -a "Launch Desk" -u normal "T−10: <mission>"` (and once at liftoff window). Debounce per launch id in cache. Off by default. | Coin Toss `notify`; `omarchy.notifications` via FreeDesktop. |
...
| 10 | **MPRIS pause cooperation** | Expose Watch via MPRIS (or pause MediaPlayer when `omarchy.media` / Coverglow issues Pause on “Launch Desk” player). At minimum: pause Watch when another MPRIS player starts (optional schema). | Coverglow + `omarchy.media`. Harder than notify; skip if sticky Watch unfinished. |
...
| Idea | Why skip |
|------|----------|
| Full **Phosphor / Lacuna CRT** overlay | Wrong product; GPU tax; competes with ambience plugins; Launch Desk already has faint scanline. |
...
| **Game loops** (Breakout/Flappy/GoL mechanics) | Dilutes launch-desk pitch; theme/keyboard/README lessons already extracted. |
...
> SpaceX launch desk on your Omarchy bar — flip-digit stats, countdown, mission detail, Watch.
